...
app.post("/auth/signup", async (c) => {
  ...
  // Generate tenant ID from company name (lowercase, hyphenated)
  ...
  const existing = await authPool.query("SELECT id FROM users WHERE email = $1", [body.email]);
  ...
  await authPool.query(
    `INSERT INTO tenants (tenant_id, system_prompt, max_tokens_per_day)
     VALUES ($1, $2, 100000)
...
app.post("/auth/login", async (c) => {
  ...
  const tenant = await getTenantConfig(user.tenant_id);
